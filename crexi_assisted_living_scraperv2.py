"""
Crexi Senior Living / Assisted Living Facility Scraper v7
==========================================================
Scrapes multiple U.S. states for senior/assisted living facilities (configured via STATES_INPUT).
...
USAGE:
    python crexi_assisted_living_scraper.py
"""
...
def _normalize_state_token(s: str) -> str:
    # Keep letters only so inputs like "NOrth Carolina", "New-Hampshire", "D.C." normalize cleanly.
    return re.sub(r"[^a-z]", "", (s or "").lower())

_US_STATE_CODES = {
    # 50 states (+ DC). Keep this mapping here so you can use either names or abbreviations.
...
# Build a lookup table that supports both abbreviations and full names (+ a few common typos).
_STATE_ALIASES = {}
...
# Common misspellings / variants seen in user inputs:
...
# State-specific Crexi URLs (Crexi expects 2-letter postal abbreviations in the URL)
STATES = _expand_states(STATES_INPUT)
SEARCH_URLS = [f"https://www.crexi.com/properties/{st}/Senior-Living" for st in STATES]


def get_output_filename():
    """Generate timestamped filename so it never conflicts with an open file."""
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"crexi_senior_living_{ts}.xlsx"
...
def scrape_search_page(page, state=""):
    ...
    card_selectors = [
        ...
        "[class*='property-card']", "[class*='AssetCard']",
        "[class*='asset-card']", "[class*='ListingCard']",
        "[class*='listing-card']", "article[class*='card']",
        "div[class*='result-card']", "div[class*='search-result']",
            ...
            for a in all_links:
                ...
                if _RE_PROPERTY_PATH.search(href) and href not in seen:
                    if any(skip in href for skip in [
                        ...
                        continue
                    full_url = href if href.startswith("http") else "https://www.crexi.com" + href
                    seen.add(href)
            ...
            try:
                ...
                try:
                    ...
                    if a:
                        ...
                        if not any(skip in h for skip in ["Senior-Living", "?types="]):
                            link = h if h.startswith("http") else "https://www.crexi.com" + h
                except Exception:
                    pass
        ...
        if _RE_PRICE.search(line) and not price:
            ...
            if sf_match:
                sqft = sf_match.group(1) + " SF"
                                     ...
                                     "continuing care", "medical care", "health care"]) and not prop_type:
            prop_type = line.strip()
                                     ...
                                     "capital", "brokerage"]) and not broker:
            broker = line.strip()

    if not address and not prop_type:
        for line in lines:
            ll = line.lower()
            if any(skip in ll for skip in ["view om", "view flyer", "save", "sign up",
    ...
    try:
        ...
        try:
            for el in page.query_selector_all("a[href^='mailto:']"):
                ...
                if email and "@" in email and "crexi" not in email:
...
def save_debug(page, body_text=""):
    try:
        ...
        with open("debug_text.txt", "w", encoding="utf-8") as f:
            f.write(body_text)
        print("  📄 Saved debug_page.html and debug_text.txt")
    except Exception:
        pass
...
def write_excel(listings, filename):
    ...
    ws["A1"] = f"Crexi — Senior / Assisted Living For Sale ({', '.join(STATES)}) — 50+ Beds / 40+ Units"
    ws["A1"].font = Font(name="Arial", bold=True, size=14, color="2F5496")
    ws["A1"].alignment = Alignment(horizontal="left", vertical="center")
    ws.row_dimensions[1].height = 32
    ws.merge_cells("A2:M2")
    ws["A2"] = f"Generated: {datetime.now().strftime('%B %d, %Y %I:%M %p')}  |  Source: crexi.com"
    ...
    # Try saving, handle permission error gracefully
    try:
        wb.save(filename)
        print(f"\n✅ Saved: {filename} ({len(listings)} listings)")
    except PermissionError:
        # If file is locked, save with a different name
...
def main():
    print("=" * 60)
    print("  Crexi Senior Living Scraper v7 (Playwright)")
    ...
    with sync_playwright() as p:
        ...
        try:
            print("Opening Crexi...")
            navigate(pg, "https://www.crexi.com")
